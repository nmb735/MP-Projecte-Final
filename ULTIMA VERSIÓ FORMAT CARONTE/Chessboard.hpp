...
ChessPieceType Chessboard::realType(char type)
{
	...
	case 'P': t = CPT_Pawn;
		break;
	default: t = CPT_EMPTY; cout << "Error en la lectura del tipus de peça. Comprovi el fitxer i torni a provar." << endl;
		break;
	}
...
bool Chessboard::MovePiece(const ChessPosition& posFrom, const ChessPosition& posTo)
{
	...
	if (trobat)
	{
		//FALTA MOVER LA PIEZA de posFrom a posTo
		//Datos de posFrom a posTo

		m_board[y2][x2].setColor(m_board[y1][x1].getPieceColor()); //desplaçem la peça
		m_board[y2][x2].setType(m_board[y1][x1].getPieceType());

		m_board[y1][x1].setColor(CPC_NONE); //buidem la peça de la que venim
...
bool Chessboard::opositionPiece(const ChessPosition& pos1, const ChessPosition& pos2) const {
	...
	int x = pos2.getPositionX(); //posicions x i y de la posicio a la que volem anar
	int y = pos2.getPositionY();

	int r = pos1.getPositionX();//posicions x i y de la posicio de la que venim
	int c = pos1.getPositionY();

	if (m_board[c][r].getPieceColor() != CPC_NONE)
	{
		if (m_board[c][r].getPieceColor() == CPC_Black) //si la nostra peça es negra, les peçes rivals son blanques
		{
		...
		else //si la nostra peça es blanca, les peçes rivals son negre
		{
...
VecOfPositions Chessboard::posKing(const ChessPosition& pos) const {
	...
	VecOfPositions v; //Vector dinamic
	ChessPosition p;
	Piece cp;
...
VecOfPositions Chessboard::posKnight(const ChessPosition& pos) const { //direccions dels comentaris en un grafic real, no de programacio, pero funciona igual
	...
	{
		v.push_back(p); //Dalt dre.
	}

	p.setTot(x - 1, y + 2);
	if (!outOfBounds(pos, p) && !teamPiece(pos, p))
	{
		v.push_back(p); //Dalt esq.
	}

	p.setTot(x + 1, y - 2);
	if (!outOfBounds(pos, p) && !teamPiece(pos, p))
	{
		v.push_back(p); //Baix dre.
	}

	p.setTot(x - 1, y - 2);
	if (!outOfBounds(pos, p) && !teamPiece(pos, p))
	{
		v.push_back(p); //Baix esq.
	}

	p.setTot(x + 2, y + 1);
	if (!outOfBounds(pos, p) && !teamPiece(pos, p))
	{
		v.push_back(p); //Dre. dalt
	}

	p.setTot(x + 2, y - 1);
	if (!outOfBounds(pos, p) && !teamPiece(pos, p))
	{
		v.push_back(p); //Dre. baix
	}

	p.setTot(x - 2, y + 1);
	if (!outOfBounds(pos, p) && !teamPiece(pos, p))
	{
		v.push_back(p); //Esq. dalt
	}

	p.setTot(x - 2, y - 1);
	if (!outOfBounds(pos, p) && !teamPiece(pos, p))
	{
		v.push_back(p); //Esq. baix
	}
...
VecOfPositions Chessboard::posRook(const ChessPosition& pos) const { //direccions dels comentaris en un grafic real, no de programacio, pero funciona igual
	...
	for (int a = 0; a < 4; a++) //4 per les direccions de la torre
	{
		int i = 0;
		int j = 0;
		bool stop = false;
...
VecOfPositions Chessboard::posBishop(const ChessPosition& pos) const { //direccions dels comentaris en un grafic real, no de programacio, pero funciona igual
	...
	for (int a = 0; a < 4; a++) //4 per les direccions de l'alfil
	{
		int i = 0;
		int j = 0;
		bool stop = false;
...
VecOfPositions Chessboard::posQueen(const ChessPosition& pos) const { //direccions dels comentaris en un grafic real, no de programacio, pero funciona igual
	...
	for (int a = 0; a < 8; a++) //4 per les direccions de la reina (juntem alfil + torre)
	{
		int i = 0;
		int j = 0;
		bool stop = false;
...
VecOfPositions Chessboard::posPawn(const ChessPosition& pos) const { //Al tauler les negres van a dalt i les blanqes a baix
	...
	if (m_board[y][x].getPieceColor() == CPC_Black)
	{
		p.setTot(x, y + 1); //Sempre pot dfer 1 moviment endevant
		if (!outOfBounds(pos, p) && !teamPiece(pos, p))
		{
			v.push_back(p);
		}

		if (y == 1 && !teamPiece(pos, p) && !opositionPiece(pos, p)) //Si esta en la pos inicial, pot fer un segon moviment, pero si hi ha una fitxa del tipus que sigui no es pot
		{
			p.setTot(x, y + 2);
			if (!outOfBounds(pos, p) && !teamPiece(pos, p))
			{
	...
	else
	{
		if (m_board[y][x].getPieceColor() == CPC_White)
		{
			...
			if (y == 6 && !teamPiece(pos, p) && !opositionPiece(pos, p)) //Si esta en la pos inicial, pot fer un segon moviment
			{
				p.setTot(x, y - 2);
				if (!outOfBounds(pos, p) && !teamPiece(pos, p))
				{
...
VecOfPositions Chessboard::GetValidMoves(const ChessPosition& pos) const
{
	...
	{
	case CPT_King://REI<-una pos en totes les direccions
		v = posKing(pos);
		break;
	case CPT_Queen://DAMA<-pos que vulguis en totes direccions
		v = posQueen(pos);
		break;
	case CPT_Rook://TORRE<-pos que vulguis en horitzontal i vertical
		v = posRook(pos);
		break;
	case CPT_Bishop://ALFIL<-pos que vulguis en diagonal
		v = posBishop(pos);
		break;
	case CPT_Knight://CAVALL<-en L o dos i un
		v = posKnight(pos);
		break;
	case CPT_Pawn://PEO<- inici 2 endevant, despres 1 endevant, 1 en diagonal per matar
		v = posPawn(pos);
		break;
	}
